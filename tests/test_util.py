...
def test_fi_output():
    test_X = pd.DataFrame({
        ...
        "ord": [0.41 , -0.08, -0.46, -0.4, -2.15, -0.29,  0.04 , -0.46,  0.58,  0.72]
    })
    ...
    test_preprocessor = make_column_transformer(
        ...
        (OrdinalEncoder(categories=[education_levels], dtype=int), ["ord"])
    )
    ...
    assert len(output.columns), "Output should have 2 columns only."
    assert len(output.data.shape[0]) == len(test_X.columns), "Number of rows should equal to number of features when `head` is not specified."
    assert len(output_head.data.shape[0]) == test_head, "Number of rows should equal to specified `head`."
    assert output.iloc[:,0].dtype == np.dtype('O'), "First column must be a feature string vector."
    assert output.iloc[:,1].dtype == np.dtype('float64'), "Second column must be a coefficient numeric vector."
    # TODO: check whether the absolute values are sorted descreasingly
